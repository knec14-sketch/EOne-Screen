...
Он поднимает права администратора, снимает с файлов пометку «скачано
из интернета» и осматривает машину: какие датчики отвечают, виден ли экран
на COM-порту, есть ли интернет и отвечает ли погода. Всё найденное
записывается в `settings.json`, и заново при каждом запуске это не ищется.

Потом уже обычный запуск:
...
Галочка «Ярлык на рабочем столе» в настройках делает ярлык, который
запускает программу с правами.
...
`start.bat` осмотрит машину и скажет, чего не хватает: библиотеки, права,
экран на порту, интернет. Читать его вывод стоит целиком — он написан
по-человечески и прямо говорит, что делать с каждой находкой.

Дальше `app.bat` открывает окно. Оно устроено просто: три раздела наверху.

**А потом каждый день — с ярлыка.** В настройках, раздел **Запуск**,
поставьте галочку «Ярлык на рабочем столе»: программа сделает ярлык
со своим значком, и он запускает её так же, как `app.bat`, — с правами.
После этого ни папка, ни командная строка больше не нужны.
...
Полное описание всех полей — в справочнике
[«Панель - справочник.md»](Панель%20-%20справочник.md).
...
Градусы Цельсия или Фаренгейта, часы на 24 или на 12 с AM и PM, порядок
чисел в дате, с какого дня начинается неделя. Настройка общая: её слушает
и окно, и то, что уходит на экран. Тему переписывать не надо — подпись `°C`
движок меняет на `°F` сам.

Важно: **в градусы переводится только то, что читают**. Значения, которыми
тема правит картинку — доля кольца, реакция на температуру, — всегда
остаются в Цельсиях. Иначе выбор шкалы сдвинул бы все пороги в теме.
