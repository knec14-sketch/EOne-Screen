...
Открытая замена штатной программе для экранов водянок **Jungle Leopard
Pro Flow 360** и **Pro Flow 240** (контроллер TXW818, панель ST7701S,
960×480, по COM-порту).
...
**Работает на Windows и на Linux.** И то и другое проверено на живом
железе: Linux — на CachyOS с KDE Plasma 6 и Wayland, экран по
`/dev/ttyACM0`, тридцать кадров в секунду.

**Говорит на семи языках:** русский, английский, испанский, немецкий,
французский, португальский, итальянский. Окно и панель могут говорить
на разных: окно — рабочее место, а панель видят другие.
...
при установке отметить «Add python.exe to PATH»). Про Linux —
[отдельно ниже](#linux).
...
Работает. Доброволец прогнал программу на **CachyOS с KDE Plasma 6
и Wayland**, с процессором Intel i5-13600KF и видеокартой Radeon
RX 7900 XT: окно, температуры, погода, темы, значок в трее и сам экран,
около 28 кадров в секунду при цели 30. [Отчёт целиком — в issue #1][issue1].

Linux-машины у автора нет, поэтому всё, что на ней ещё не запускали,
ниже помечено.
...
`pythonnet` на Linux не нужен вовсе, и `start.bat` тоже — он снимает
пометку, которой в Linux не бывает.

**Доступ к экрану.** Читать последовательный порт даёт группа:
`dialout` в Debian и Ubuntu, `uucp` в Arch и его родне (CachyOS,
Manjaro).

```
sudo usermod -a -G uucp $USER      # в Debian и Ubuntu: dialout
```

Потом выйти из системы и войти снова.

**Значок возле часов.** Ему нужен AppIndicator, а тот ставится системой,
а не через `pip`:
...
Если работаете в отдельной среде, заводите её с ключом
`--system-site-packages` — иначе модуль `gi` из неё не виден и значка
не будет:
...
Без значка программа тоже работает — просто закрывается крестиком,
а не прячется.

Всё, что зависит от системы, лежит в одном файле, `sistema.py`:

| | Windows | Linux |
|---|---|---|
| температуры | LibreHardwareMonitor, нужны права | `/sys/class/hwmon`, прав не нужно |
| загрузка, память и мощность видеокарты | библиотека датчиков | `/sys/class/drm` |
| имя процессора | реестр | `/proc/cpuinfo` |
| имя видеокарты | WMI | `nvidia-smi`, иначе `lspci` |
| единицы и форматы | реестр | локаль |
| ярлык и автозапуск | WScript.Shell и реестр | файлы `.desktop` |
| библиотека датчиков | четыре `.dll` | не нужна вовсе |

**Какие экраны узнаются.** Два: `33C3:7788` (Flow 360) и `33C3:7792`
(Flow 240, панель HONGTAI). Второй добавлен после проверки на живом
железе — протокол оказался тот же самый. Остальные коды намеренно
не вписаны, пока их так же не подтвердят.

**Что на живом железе ещё не запускали:** показания видеокарты из
`/sys/class/drm` — загрузка, видеопамять, мощность; разбор проверен
на подложенных файлах, но не на настоящей карте. Туда же автозапуск
после перезагрузки и ярлык в меню приложений. Если у вас что-то
из этого работает или не работает — [заведите issue][issues].

Разбор проверяется так: `python проверки/linux.py`.
...
Семь: русский, английский, испанский, немецкий, французский,
португальский, итальянский. Переключаются в настройках, применяются сразу.

У **окна** и у **темы** настройки разные. Окно — рабочее место, а панель
висит на виду, и языки им нужны не обязательно одни и те же. Настройка
темы правит всем, что теме принадлежит: слова погоды, подпись ветра,
названия слоёв в редакторе. По умолчанию тема идёт за окном.

Каждый язык лежит в своём файле — `yazyk_es.py` и такие же рядом:
словарь окна, слова погоды и их сокращения. Ключ словаря — сама русская
строка, поэтому пропущенная строка отступает на английский, а потом
на русский: недоделанный перевод ничего не ломает.

> **Переводы на испанский, немецкий, французский, португальский
> и итальянский сделаны с помощью ИИ и не проверены носителями языка.**
> Русский и английский — авторские. Если вы говорите на одном из этих
> пяти и видите ошибку или просто корявость, [заведите issue][issues]:
> поправка от того, кто на языке говорит, стоит больше любой машинной
> аккуратности.
...
Тема «Skywatch» носит с собой свой шрифт — **Project Space** работы Ver Wave,
под SIL Open Font License 1.1. Он лежит в `themes/Skywatch/fonts/` вместе
с текстом лицензии, поэтому тема выглядит одинаково на любой машине
